...
class Options():
    def initialize(self, parser):
        ...
        # ============ Wavelet-Gated-Former settings ============
        # 新增的Hierarchical Wavelet-Gated-Former模型配置
        parser.add_argument('--model_type', type=str, default='forgelens', 
                            choices=['forgelens', 'wavelet_gated_former'],
                            help='Model type: forgelens (original) or wavelet_gated_former (new)')
        
        # Hierarchical Wavelet-Head settings
        parser.add_argument('--num_wavelet_heads', type=int, default=12,
                            help='Number of wavelet heads (layers to extract from ViT)')
        parser.add_argument('--wavelet_type', type=str, default='db6',
                            help='Wavelet type (e.g., db6, haar, sym)')
        parser.add_argument('--wavelet_J', type=int, default=3,
                            help='Number of wavelet decomposition levels')
        
        # GFFM (Gated Frequency Fusion Module) settings
        parser.add_argument('--gffm_transformer_layers', type=int, default=2,
                            help='Number of transformer layers in GFFM')
        parser.add_argument('--gffm_transformer_heads', type=int, default=2,
                            help='Number of attention heads in GFFM')
        parser.add_argument('--gffm_reduction_factor', type=int, default=1,
                            help='MLP reduction factor in GFFM')
        
        # Wavelet-Gated-Former training settings
        parser.add_argument('--wgf_batch_size', type=int, default=16,
                            help='Batch size for Wavelet-Gated-Former')
        parser.add_argument('--wgf_epochs', type=int, default=30,
                            help='Number of epochs for Wavelet-Gated-Former')
        parser.add_argument('--wgf_learning_rate', type=float, default=2e-6,# 学习率：1e-6——>1e-5——>2e-6
                            help='Learning rate for Wavelet-Gated-Former')
        parser.add_argument('--wgf_lr_decay_step', type=int, default=5,
                            help='LR decay step for Wavelet-Gated-Former')
        parser.add_argument('--wgf_lr_decay_factor', type=float, default=0.7,
                            help='LR decay factor for Wavelet-Gated-Former')
        parser.add_argument('--resume', action='store_true',
                            help='Resume training from the latest checkpoint')
        
        # ============ Improved Gating settings ============
        # 改进门控机制的参数（用于 train_wavelet_gated_former_improved.py）
        parser.add_argument('--gate_lr_multiplier', type=float, default=50.0,
                            help='Learning rate multiplier for gate weights (default: 50)')
        parser.add_argument('--diversity_weight', type=float, default=0.1,
                            help='Weight for diversity regularization loss (default: 0.1)')
        parser.add_argument('--diversity_loss_type', type=str, default='variance',
                            choices=['variance', 'entropy', 'l2'],
                            help='Type of diversity loss (default: variance)')
        parser.add_argument('--use_improved_gffm', type=lambda x: x.lower() == 'true', 
                            default=True,
                            help='Use improved GFFM with random initialization (default: True)')
